...
export default function RelatorioVendasPage() {
  ...
  const buscar = (ini?: string, fim?: string) => {
    ...
    setLoading(true);
    api.get<RelVendas>(`/relatorios/vendas?${params}`).then(setData).finally(() => setLoading(false));
  };

  // Agrupar por cliente
  const porCliente: Record<number, { nome: string; total: number; quantidade: number }> = {};
  const porVendedor: Record<number, { nome: string; total: number; quantidade: number }> = {};
  const porProduto: Record<number, { nome: string; quantidade: number; total: number; unidade: string }> = {};

  data?.vendas.forEach(v => {
    // Por cliente
    if (!porCliente[v.clienteId]) porCliente[v.clienteId] = { nome: v.cliente.nomeFantasia || v.cliente.razaoSocial, total: 0, quantidade: 0 };
    porCliente[v.clienteId].total += parseFloat(String(v.valorTotal));
    porCliente[v.clienteId].quantidade++;
    // Por vendedor
    if (!porVendedor[v.vendedorId]) porVendedor[v.vendedorId] = { nome: v.vendedor.nome, total: 0, quantidade: 0 };
    porVendedor[v.vendedorId].total += parseFloat(String(v.valorTotal));
    porVendedor[v.vendedorId].quantidade++;
    // Por produto
    v.itens?.forEach(item => {
      if (!porProduto[item.produtoId]) porProduto[item.produtoId] = { nome: item.produto.nome, quantidade: 0, total: 0, unidade: item.produto.unidade };
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="card p-4 text-center">
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{data.quantidade}</p>
            </div>
            <div className="card p-4 text-center">
              <p className="text-sm text-gray-500">Faturamento total</p>
              <p className="text-3xl font-bold text-green-700 mt-1">{formatMoney(data.totalFaturamento)}</p>
            </div>
            <div className="card p-4 text-center">
              <p className="text-sm text-gray-500">Ticket médio</p>
              <p className="text-3xl font-bold text-blue-600 mt-1">
                {data.quantidade > 0 ? formatMoney(data.totalFaturamento / data.quantidade) : '-'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
            ...
            <div className="card overflow-hidden">
              ...
              <table className="w-full">
                <thead><tr className="border-b border-gray-100"><th className="table-header">Cliente</th><th className="table-header text-right">Qtd</th><th className="table-header text-right">Total</th></tr></thead>
                <tbody>
                  {Object.values(porCliente).sort((a, b) => b.total - a.total).map((c, i) => (
            ...
            <div className="card overflow-hidden">
              ...
              <table className="w-full">
                <thead><tr className="border-b border-gray-100"><th className="table-header">Produto</th><th className="table-header text-right">Quantidade</th><th className="table-header text-right">Total</th></tr></thead>
                <tbody>
