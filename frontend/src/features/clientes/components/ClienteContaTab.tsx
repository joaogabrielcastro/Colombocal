...
export function ClienteContaTab({ conta, clienteId, freteEnabled, fretesPendentes, pgFreteTipo, setPgFreteTipo, pgFreteData, setPgFreteData, pagandoFreteId, onPagarFrete }: Props) {
  return <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <div className="card"><div className="px-5 py-3 border-b border-gray-100 flex justify-between items-center"><h3 className="font-semibold text-gray-900">Pagamentos ({conta.pagamentos.length})</h3><Link href={`/financeiro/novo?clienteId=${clienteId}`} className="text-blue-600 text-xs hover:underline">+ Receber</Link></div>
